    ...
    <div>
      ...
      <ul>
        {filteredTrainers.map(trainer => (
          <li key={trainer._id}>
            <h3>
              <a href={`/appointment?trainer=${encodeURIComponent(trainer.name)}`}>
                {trainer.name}
              </a>
            </h3>
        ...
        {filteredCourses.map(course => {
          ...
          return (
            <li key={course._id}>
              ...
              <p>Trainers: {teachingTrainers.map(trainer => (
                <a key={trainer._id} href={`/appointment?trainer=${encodeURIComponent(trainer.name)}`}>
                  {trainer.name}
                </a>
